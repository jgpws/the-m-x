...
?>
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		<header class="entry-header">
			<?php
				if ( is_single() ) {
					the_title( '<h1 class="entry-title"><i class="material-icons">' . esc_html__( 'view_comfy', 'the-m-x' ) . '</i>', '</h1>' );
				} else {
					the_title( '<h2 class="entry-title"><i class="material-icons">' . esc_html__( 'view_comfy', 'the-m-x' ) . '</i>', '</h2>' );
				}
