function skrollrDataAttributes() {
	/* For skroller.js effects */
		...
		var galleryItem = document.querySelectorAll( '.blog .gallery-item, .blog .blocks-gallery-item' );
		...
		//var cardContentGallery = document.querySelectorAll( '.blog .format-standard.has-gallery .entry-content', '.blog .format-standard.has-gallery-block .entry-content', '.blog .format-standard.has-gallery .entry-summary', '.blog .format-standard.has-gallery-block .entry-summary' );
		...
		if ( bodyTag.classList.contains( 'skrollr-animate' ) ) {
			...
			if ( mxSkrollrParams.layout_type === 'centered' || mxSkrollrParams.layout_type === 'wide' ) {
				...
				for ( i = 1; i < cardContent.length; i++ ) {
					if ( card[i].classList.contains( 'has-gallery-block' ) || card[i].classList.contains( 'has-gallery' ) ) {
						cardContent[i].setAttribute( 'data-0-bottom-center', 'transform: translateX(0%); opacity: 0' );
						cardContent[i].setAttribute( 'data-144-center-center', 'transform: translateX(0%); opacity: 1.0' );
						cardContent[cardContent.length - 1].setAttribute( 'data-0-bottom-center', 'opacity: 1.0' );
					} else {
					...
					console.log(cardContent[i].classList);
				}

				/*for ( i = 1; i < cardContentGallery.length; i++ ) {
					cardContentGallery[i].setAttribute( 'data-0-bottom-center', 'transform: translateX(0%); opacity: 0' );
					cardContentGallery[i].setAttribute( 'data-144-center-center', 'transform: translateX(0%); opacity: 1.0' );
					cardContentGallery[cardContentGallery.length - 1].setAttribute( 'data-0-bottom-center', 'opacity: 1.0' );
				}*/
